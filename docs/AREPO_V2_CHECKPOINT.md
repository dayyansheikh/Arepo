...
- Exact next action: commit validated D036 and run the single planned finite attempt using the implemented separately gated finite 4,000-request capacity protocol in `docs/implementation/PHASE_03_CAPACITY_REFINEMENT.md`. Verify current disk/RAM; retain existing defaults, require actual prior request-ceiling proof, test/self-review/commit before any new source request. The sampler now explicitly supports at most 400,000 members and its synthetic sizing passed. Do NOT rerun unchanged 1,000-page enumeration. Original-build consumption is resolved by D034 and actual read evidence. After a usable complete interval frame exists, follow the panel journal contract for durable protocol/seed/sample/actual read/computation/origin records, collectors/targets and the pilot. Phase 4 remains gated.
