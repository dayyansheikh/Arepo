...
Updated: 2026-09-22. Status: Phase 3 incomplete — complete-frame gate cleared; exact metadata projection validated. Durable selection/origin/pilot work remains. Phases 0–2 complete. Saving a safe boundary near the five-hour usage limit; scheduled continuation remains active.
...
- Latest safe implementation commit: `c44380d42cd3e988360cc685914f9d529a7d088b` — D039 exact metadata, **818 tests passed**. Complete-frame evidence commit `13dd2cb`; original measurement implementation remains `41fcb0170c7d71716873847639dc181ea12422aa` (D038). Accepted Phase 2 `747485c`. This final recovery-record commit changes documentation only.
...
- Tests/results: latest full backend **818 passed**, zero skipped, 74.04s, including actual disposable PostgreSQL 17.11, 23 new metadata cases and all 11 connection cases. Full Ruff, canonical contract checker and whitespace checks pass. One existing Starlette/httpx warning. Local test DBs only; email disabled. Initial lifecycle-wrapper and session-integrity failures were fixed without weakening the build guard or tests.
...
- Decisions: D000–D039. D038 adds opt-in bounded connection reuse; defaults and source/budget/retry rules stay unchanged. D039 adds pure exact metadata projection, with explicit present/missing/invalid states and no fallback, eligibility exclusion or origin admission. New modules/tests: research_panel/metadata.py and tests/unit/test_research_panel_metadata.py. Old failures remain failed; source events are not independent economic groups.
- Required data gate: complete-frame blocker **cleared** by verified attempt 5. The four earlier failures remain incomplete. There is no current protected approval/access blocker. This is an interval enumeration, not atomicity, economic independence or predictive evidence. Latest allowance check: 92% primary /76% weekly used; ordinary usage still available, shutdown checkpoint before exhaustion. No reset credit redeemed.
...
- Exact next action: reload PHASE_03_PANEL_JOURNAL_CONTRACT.md and integrate the tested pure metadata projector into durable policy/seed/full-row inventory/actual-read/selection journals. Freeze policy/seed before selection reads; consume the complete frame through original commit `41fcb0170c7d71716873847639dc181ea12422aa`; never recollect merely because code changed. Preserve all 44 unresolved rows and unknown metadata. Record current actual read/projection clocks separately from old availability. Define freshness before any future origin; this historical frame is development evidence, not backdated origins. Then collectors/targets and predeclared pilot. Phase 4 remains gated.
